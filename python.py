...
try:
    ...
    #  create a new cursor
    cur = con.cursor()
    read_table = """SELECT date_Trunc('month', "published_at"::date) date, count(distinct "UID") articles from news_log where length("published_at") > 21 group by 1 having count(distinct "UID") > 100  order by 1;"""
